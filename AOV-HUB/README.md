...
│   ├── register.html            # Đăng ký                — Người 1
│   ├── login.html                # Đăng nhập              — Người 1
│   └── profile.html             # Trang cá nhân (có Guard) — Người 1
│
...
│   ├── auth.css                  # Đăng ký/Đăng nhập/Profile — Người 1
...
│   ├── auth.js                   # Đăng ký/Đăng nhập/Session/Guard — Người 1
...
aov_users         # [{ username, password, createdAt }, ...] — tài khoản đăng ký (mô phỏng, không mã hoá)
aov_current_user  # username đang đăng nhập (nếu có) — quản lý bởi js/auth.js
```

Không tự ý đổi tên các key trên khi thêm tính năng mới — các file `storage.js`, `favorite.js`, `compare.js`, `auth.js` đều phụ thuộc vào key cố định này.
...
|---|---|
| Người 1 | `reset.css`, `variables.css`, `common.css`, `home.css`, `auth.css`, `partials/`, `js/loadData.js`, `js/components.js`, `js/layout.js`, `js/home.js`, `js/auth.js`, `index.html`, `pages/register.html`, `pages/login.html`, `pages/profile.html` |
