 ...
 * Thay bằng placeholder SVG để tránh hiện icon "ảnh vỡ" trên toàn site.
 */
function handleImageError(img) {
    ...
    img.src = 'data:image/svg+xml;utf8,' +
        encodeURIComponent(
            ...
            '<rect width="100%" height="100%" fill="#221a38"/>' +
    ...
    return `
        <article class="card hero-card">
            <a class="hero-card__media" href="/pages/hero-detail.html?id=${hero.id}">
                <img src="/assets/images/${hero.image}" alt="${hero.name}" loading="lazy" onerror="handleImageError(this)">
            </a>
            <button
                type="button"
                class="btn-favorite"
    ...
    return `
        <article class="card item-card">
            <a class="item-card__media" href="/pages/item-detail.html?id=${item.id}">
                <img src="/assets/images/${item.image}" alt="${item.name}" loading="lazy" onerror="handleImageError(this)">
            </a>
            <button
                type="button"
                class="btn-favorite"
